...
For $j=1 To $filelist[0]
   ConsoleWrite($filelist[$j]&" begin ")
   $filepath = "data/luajit/" & $filelist[$j]
   $asmpath = "data/asm/" & $filelist[$j] & ".asm"
   $savepath1 = "data/out/out_" & $filelist[$j]
   decompfile($filepath,$asmpath,$savepath1)
   ConsoleWrite($filelist[$j]&" end ")
Next

Func decompfile($filepath,$asmpath,$savepath1)
